import { useState, useEffect } from 'react';
import { useParams, Link, useNavigate } from 'react-router-dom';
import { MainLayout } from '@/components/layout/MainLayout';
import { useEquipmentCatalog } from '@/hooks/useEquipmentCatalog';
import { EquipmentCatalog, CatalogRegister } from '@/types/catalog';
import { CatalogForm } from '@/components/catalog/CatalogForm';
import { ProtocolSection } from '@/components/catalog/ProtocolSection';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import {
  AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent,
  AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle,
} from '@/components/ui/alert-dialog';
import { ChevronLeft, Pencil, Trash2, BookOpen, Network, Variable, Loader2 } from 'lucide-react';
import { toast } from 'sonner';

const EquipmentCatalogDetail = () => {
  const { catalogId } = useParams<{ catalogId: string }>();
  const navigate = useNavigate();
  const {
    fetchCatalogDetail, updateCatalog, deleteCatalog,
    addProtocol, updateProtocol, deleteProtocol,
  } = useEquipmentCatalog();

  const [catalog, setCatalog] = useState<EquipmentCatalog | null>(null);
  const [loading, setLoading] = useState(true);
  const [editOpen, setEditOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deleting, setDeleting] = useState(false);

  const loadCatalog = async () => {
    if (!catalogId) return;
    setLoading(true);
    const data = await fetchCatalogDetail(catalogId);
    setCatalog(data);
    setLoading(false);
  };

  useEffect(() => { loadCatalog(); }, [catalogId]);

  const handleUpdate = async (data: { manufacturer: string; model: string; description?: string }) => {
    if (!catalogId) return;
    setSaving(true);
    try {
      await updateCatalog(catalogId, data);
      toast.success('Catalog updated!');
      setEditOpen(false);
      await loadCatalog();
    } catch (error: any) {
      if (error?.code === '23505') {
        toast.error('A catalog with this manufacturer and model already exists.');
      } else {
        toast.error('Error updating catalog');
      }
    }
    setSaving(false);
  };

  const handleDelete = async () => {
    if (!catalogId) return;
    setDeleting(true);
    try {
      await deleteCatalog(catalogId);
      toast.success('Catalog deleted');
      navigate('/equipment-catalog');
    } catch (error) {
      toast.error('Error deleting catalog');
    }
    setDeleting(false);
  };

  const handleAddProtocol = async (protocol: string, registers: CatalogRegister[]) => {
    if (!catalogId) return;
    try {
      await addProtocol(catalogId, protocol, registers);
      toast.success(`Protocol "${protocol}" added with ${registers.length} registers!`);
      await loadCatalog();
    } catch (error: any) {
      if (error?.code === '23505') {
        toast.error('This protocol already exists for this catalog.');
      } else {
        toast.error('Error adding protocol');
      }
    }
  };

  const handleUpdateProtocol = async (protocolId: string, registers: CatalogRegister[]) => {
    try {
      await updateProtocol(protocolId, registers);
      toast.success(`Registers re-imported (${registers.length} registers)`);
      await loadCatalog();
    } catch (error) {
      toast.error('Error updating protocol');
    }
  };

  const handleDeleteProtocol = async (protocolId: string) => {
    try {
      await deleteProtocol(protocolId);
      toast.success('Protocol deleted');
      await loadCatalog();
    } catch (error) {
      toast.error('Error deleting protocol');
    }
  };

  if (loading) {
    return (
      <MainLayout>
        <div className="p-8 flex items-center justify-center min-h-[60vh]">
          <Loader2 className="h-8 w-8 animate-spin text-muted-foreground" />
        </div>
      </MainLayout>
    );
  }

  if (!catalog) {
    return (
      <MainLayout>
        <div className="p-8">
          <div className="text-center py-12">
            <h2 className="text-2xl font-bold">Catalog not found</h2>
            <Link to="/equipment-catalog" className="text-[#2563EB] hover:underline mt-2 inline-block">
              Back to Equipment Catalog
            </Link>
          </div>
        </div>
      </MainLayout>
    );
  }

  return (
    <MainLayout>
      <div className="p-8">
        <Link
          to="/equipment-catalog"
          className="inline-flex items-center text-sm text-muted-foreground hover:text-slate-900 mb-4"
        >
          <ChevronLeft className="h-4 w-4 mr-1" />
          Back to Equipment Catalog
        </Link>

        {/* Header */}
        <div className="mb-8">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-lg bg-blue-100">
                <BookOpen className="h-8 w-8 text-[#2563EB]" />
              </div>
              <div>
                <h1 className="text-3xl font-bold text-[#1a2744]">
                  {catalog.manufacturer} / {catalog.model}
                </h1>
                {catalog.description && (
                  <p className="text-muted-foreground mt-1">{catalog.description}</p>
                )}
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="outline" onClick={() => setEditOpen(true)}>
                <Pencil className="h-4 w-4 mr-2" />Edit
              </Button>
              <Button
                variant="outline"
                className="text-red-500 hover:text-red-700 hover:bg-red-50"
                onClick={() => setDeleteDialogOpen(true)}
              >
                <Trash2 className="h-4 w-4 mr-2" />Delete
              </Button>
            </div>
          </div>
        </div>

        {/* Stats */}
        <div className="grid gap-4 md:grid-cols-3 mb-8">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Protocols</CardTitle>
              <Network className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{catalog.protocols?.length || 0}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total Registers</CardTitle>
              <Variable className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{catalog.total_registers || 0}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Manufacturer</CardTitle>
              <BookOpen className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-lg font-medium">{catalog.manufacturer}</div>
              <div className="text-sm text-muted-foreground">{catalog.model}</div>
            </CardContent>
          </Card>
        </div>

        {/* Protocols */}
        <Card>
          <CardContent className="pt-6">
            <ProtocolSection
              protocols={catalog.protocols || []}
              onAddProtocol={handleAddProtocol}
              onUpdateProtocol={handleUpdateProtocol}
              onDeleteProtocol={handleDeleteProtocol}
            />
          </CardContent>
        </Card>

        {/* Edit Dialog */}
        <CatalogForm
          open={editOpen}
          onOpenChange={setEditOpen}
          catalog={catalog}
          onSave={handleUpdate}
          saving={saving}
        />

        {/* Delete Dialog */}
        <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Delete catalog?</AlertDialogTitle>
              <AlertDialogDescription>
                This will permanently delete "{catalog.manufacturer} / {catalog.model}" and all its protocols and register maps. Equipment linked to this catalog will be unlinked, but applied semantics will be preserved.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancel</AlertDialogCancel>
              <AlertDialogAction
                onClick={handleDelete}
                disabled={deleting}
                className="bg-red-600 hover:bg-red-700"
              >
                {deleting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Trash2 className="h-4 w-4 mr-2" />}
                Delete
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
    </MainLayout>
  );
};

export default EquipmentCatalogDetail;