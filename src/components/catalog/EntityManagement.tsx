...
import { Plus, Trash2, Loader2, Factory, Box, Network, FileJson, Search } from 'lucide-react';
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Manage manufacturers, equipment models, and supported protocols</CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="manufacturers" className="space-y-4">
            <div className="flex items-end gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => setMfgJsonOpen(true)}>
                <FileJson className="h-4 w-4 mr-1" />Import
              </Button>
            </div>
                      ...
                      <TableCell>
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-red-500" disabled={deletingId === m.id}>
                              {deletingId === m.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}
                            </Button>
                          </AlertDialogTrigger>
                          <AlertDialogContent>
                            <AlertDialogHeader><AlertDialogTitle>Delete "{m.name}"?</AlertDialogTitle><AlertDialogDescription>This will also delete all models linked to this manufacturer.</AlertDialogDescription></AlertDialogHeader>
                            <AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => handleDeleteMfg(m.id)} className="bg-red-600">Delete</AlertDialogAction></AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
                      </TableCell>
                    </TableRow>
          ...
          <TabsContent value="models" className="space-y-4">
            <div className="flex items-end gap-2 flex-wrap">
              <div className="space-y-1 w-48">
                ...
                <Select value={modelMfgId} onValueChange={setModelMfgId}>
              ...
              <Button onClick={handleAddModel} disabled={!modelMfgId || !modelName.trim() || modelAdding} size="sm">
                {modelAdding ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4 mr-1" />}Add
              </Button>
              <Button variant="outline" size="sm" onClick={() => setModelJsonOpen(true)}>
                <FileJson className="h-4 w-4 mr-1" />Import
              </Button>
            </div>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Input placeholder="Search..." value={modelSearch} onChange={e => setModelSearch(e.target.value)} className="pl-10 h-8 text-xs" />
            </div>
            <div className="rounded-md border max-h-[300px] overflow-auto">
              <Table>
                      ...
                      <TableRow key={m.id}>
                        ...
                        <TableCell>
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader><AlertDialogTitle>Delete "{m.name}"?</AlertDialogTitle></AlertDialogHeader>
                              <AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => handleDeleteModel(m.id)} className="bg-red-600">Delete</AlertDialogAction></AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                        </TableCell>
                      </TableRow>
          ...
          <TabsContent value="protocols" className="space-y-4">
            <div className="flex items-end gap-2">
              <div className="flex-1 space-y-1">
                ...
                <Input placeholder="e.g., Modbus TCP" value={protoName} onChange={e => setProtoName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddProto()} />
              </div>
              <div className="flex-1 space-y-1">
                <Label className="text-xs">Description (optional)</Label>
                <Input placeholder="e.g., Standard Modbus over TCP/IP" value={protoDesc} onChange={e => setProtoDesc(e.target.value)} />
              </div>
              <Button onClick={handleAddProto} disabled={!protoName.trim() || protoAdding} size="sm">
                {protoAdding ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4 mr-1" />}Add
              </Button>
              <Button variant="outline" size="sm" onClick={() => setProtoJsonOpen(true)}>
                <FileJson className="h-4 w-4 mr-1" />Import
              </Button>
            </div>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Input placeholder="Search..." value={protoSearch} onChange={e => setProtoSearch(e.target.value)} className="pl-10 h-8 text-xs" />
            </div>
            <div className="rounded-md border max-h-[300px] overflow-auto">
              <Table>
                  ...
                  {filteredProtos.length === 0 ? (
                    ...
                    <TableRow key={p.id}>
                      ...
                      <TableCell>
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-red-500" disabled={deletingId === p.id}>
                              {deletingId === p.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}
                            </Button>
                          </AlertDialogTrigger>
                          <AlertDialogContent>
                            <AlertDialogHeader><AlertDialogTitle>Delete "{p.name}"?</AlertDialogTitle></AlertDialogHeader>
                            <AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => handleDeleteProto(p.id)} className="bg-red-600">Delete</AlertDialogAction></AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
                      </TableCell>
                    </TableRow>
      ...
      <Dialog open={mfgJsonOpen} onOpenChange={setMfgJsonOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Import Manufacturers</DialogTitle><DialogDescription>Paste a JSON array of names: <code>["Schneider Electric", "ABB", "Siemens"]</code></DialogDescription></DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setMfgJsonOpen(false)}>Cancel</Button>
            <Button onClick={handleImportMfg} disabled={!mfgJson.trim() || mfgImporting} className="bg-[#2563EB]">
              {mfgImporting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : null}Import
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={modelJsonOpen} onOpenChange={setModelJsonOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Import Models</DialogTitle><DialogDescription>JSON array: <code>{`[{"manufacturer":"ABB","model":"PM5560","description":"..."}]`}</code></DialogDescription></DialogHeader>
          <Textarea value={modelJson} onChange={e => setModelJson(e.target.value)} placeholder={'[\n  {"manufacturer": "ABB", "model": "PM5560"},\n  {"manufacturer": "Schneider", "model": "ION7650"}\n]'} className="font-mono text-xs min-h-[150px]" />
          <DialogFooter>
            <Button variant="outline" onClick={() => setModelJsonOpen(false)}>Cancel</Button>
            <Button onClick={handleImportModels} disabled={!modelJson.trim() || modelImporting} className="bg-[#2563EB]">
              {modelImporting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : null}Import
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={protoJsonOpen} onOpenChange={setProtoJsonOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Import Protocols</DialogTitle><DialogDescription>JSON array: <code>{`["Modbus TCP", "Modbus RTU"]`}</code> or <code>{`[{"name":"Modbus TCP","description":"..."}]`}</code></DialogDescription></DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setProtoJsonOpen(false)}>Cancel</Button>
            <Button onClick={handleImportProtos} disabled={!protoJson.trim() || protoImporting} className="bg-[#2563EB]">
              {protoImporting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : null}Import
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
